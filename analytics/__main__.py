...
def run() -> None:
    ...
    teams_meta = {
        r["team_id"]: r
        for r in _rows(con, "select team_id, team_name, team_tla, team_crest_url from main.dim_teams")
    }
    con.close()
    ...
    agg = {t: {"gf": 0, "ga": 0, "xgf": 0.0, "xga": 0.0, "pts": 0, "xpts": 0.0,
               "played": 0, "opp_rating_sum": 0.0} for t in model.teams}
    for m in finished:
    ...
    for t in model.teams:
        ...
        team_strength.append({
            "team_id": t, "team_name": name, "team_tla": tla, "crest": crest,
            ...
            "xgf": round(d["xgf"], 2), "xga": round(d["xga"], 2),
            "points": d["pts"], "xpoints": round(d["xpts"], 2),
    ...
    for m in finished:
        ...
        upsets.append({
            ...
            "winner_name": wn, "winner_crest": wc,
            "loser_name": ln, "loser_crest": lc,
    ...
    if spain_id is not None:
        ...
        _write("spain_case.json", {
            "team_name": s["team_name"], "crest": s["crest"],
            "rank_overall": s["rank_overall"], "rank_attack": s["rank_attack"],
            "rank_defense": s["rank_defense"], "n_teams": len(model.teams),
            "goals_for": s["goals_for"], "xgf": s["xgf"],
            "goals_against": s["goals_against"], "xga": s["xga"],
